...
#LAYER_THICKNESS = 1
pitch_true = 1/(1/(pitch_z * LAYER_THICKNESS) **2 + 1/pitch_x**2)**0.5
print("pitch : {} nm".format(pitch_true * PIXELSIZE))
print("pitch * n : {} nm".format(pitch_true * PIXELSIZE * no))

#size_x = pitch_x * 1 * SCALE[1]
#size_z = pitch_z * 2 * SCALE[0]
size_x = pitch_x * 1
size_z = pitch_z * 2 
...
d = np.ones(shape = (NLAYERS,)) * LAYER_THICKNESS
...
# epsv, epsa = data.filter_epsva(epsv[...,None,:,:],epsa[...,None,:,:],wave.k0(400,PIXELSIZE),betamax = betamax)
# epsv = data.uniaxial_order(1., epsv[...,0,:,:])
# epsa = epsa[...,0,:,:]
...
if __name__ == "__main__":
    ...
    #fmode_in_ref = tmm2d.unlist_modes(tmm2d.project2d(fmode_in_listed, fmatin))
    ...
    viewer1 = dtmm.field_viewer(field_data_in, mode = "r", n = 1.5, focus = 0,intensity = 1, cols = cols,rows = rows, analyzer = "h", betamax = betamax)
    viewer2 = dtmm.field_viewer(field_data_out, mode = "t", n = 1.5, focus = 0,intensity = 1, cols = cols,rows = rows, analyzer = "h", betamax = betamax)
    viewer3 = dtmm.field_viewer(field_data_in, mode = "t", n = 1.5, focus = 0,intensity = 1, cols = cols,rows = rows, analyzer = "h", betamax = betamax)
